...
function reduzirStr($str,$quantidade){
  ...
  function cadastrar($nome,$email,$peso,$altura,$imc,$classificacao)
    ...
    function cadastrarCuriosidade($titulodacuriosidade,$curiosidade)
    {
       
        if(!$titulodacuriosidade  || !$curiosidade){return;}
        $sql = "INSERT INTO `curiosidades_tb` (`titulo`,`curiosidade` )
        VALUES(:titulo,:curiosidade)";
        $pdo = Database::conexao();
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':titulo', $titulodacuriosidade);
        $stmt->bindParam(':curiosidade', $curiosidade);
        $result = $stmt->execute();
        return ($result)?true:false;
    }
    ...
    function listarCuriosidades()
    {
        $pdo = Database::conexao();
        $sql = "SELECT * FROM curiosidades_tb";
        $stmt = $pdo->prepare($sql);
        $list = $stmt->execute();
        $list = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $list;
